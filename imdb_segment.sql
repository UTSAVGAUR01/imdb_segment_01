...
WITH hindi_movies_average_duration AS (
    ...
    FROM movie
    WHERE movie.country LIKE "%India%" AND languages LIKE '%Hindi%'
),
other_languages_average_duration AS (
    SELECT AVG(duration) AS other_languages_average
    FROM movie
    WHERE movie.country LIKE "%India%" AND languages NOT LIKE '%Hindi%'
)
...
SELECT
    AVG(CASE WHEN movie.country LIKE "%India%" languages LIKE '%Hindi%' THEN duration END) AS hindi_average,
    AVG(CASE WHEN movie.country LIKE "%India%" languages NOT LIKE '%Hindi%' THEN duration END) AS other_languages_average
FROM movie
WHERE duration IS NOT NULL;

--- Ans: Hindi Average:- 126.2515 || Other Language average: 135.4107
